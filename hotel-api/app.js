...
const hotels = [
    {
        ...
        "address":"Gurugram, India",
        "hotelID": "ht456a002",
        "description": "FabHotel Tavishk is among the most preferred budge hotels on Golf Course Road, Gurgaon."
        },
        {
        "title": "FabHotel The Residency",
        "address":"Gurugram, India",
        "hotelID": "ht456a003",
        "description": "Situated in the safe and peaceful locality of SushantLok I, FabHotel The Residency DLF Galleria provides a convenientbudget accommodation to several business travelers in Gurgaon."
        },
        {
        "title": "FabHotel Hill View Suites",
        "address":"Gurugram, India",
        "hotelID": "ht456a004",
        "description": "Situated in the posh locality of DLF Phase 2, FabHotelHill View Suites DLF Phase 2 offers a convenient and comfortableaccommodation for business travelers."
        },
        {
        ...
        "address":"Chennai, India",
        "hotelID": "ht456a011",
        "description": "FabHotel Sri Chakra Inn offers ultimate comfort tovisitors by rendering all essential amenities at low prices."
        },
        {
        ...
        "address":"Noida, India",
        "hotelID": "ht456a013",
        "description": "Strategic location, tastefully decorated rooms and and homely atmosphere is what defines FabHotel White Castle."
        },
        {
        ...
        "address":"Pune, India",
        "hotelID": "ht456a018",
        "description": "Spick and span rooms, warm hospitality and primelocation make FabHotel Atithi Baner one of the preferred hotels in Pune."
        },
...
app.get('/api/locations', (req, res) => {
    const locations = [...new Set(hotels.map((hotel) => hotel.address))];
    res.json(locations);
  });

app.get('/api/hotels/byLocation/:location', (req, res) => {
    const { location } = req.params;
    const matchingHotels = hotels.filter((hotel) => hotel.address === location);
    res.json({ location, hotels: matchingHotels });
  });
  
  // API endpoint to search for hotels by name
  app.get('/api/hotels/byName/:name', (req, res) => {
    const { name } = req.params;
    const matchingHotels = hotels.filter((hotel) =>
      hotel.title.toLowerCase().includes(name.toLowerCase())
    );
